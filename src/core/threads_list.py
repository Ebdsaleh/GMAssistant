# src.core.threads_list.py

"""
this class creates and edits the Threads List, story threads that go in the Adventure List.
"""
import random
...
class ThreadsList:
    ...
    def add_item(self, text):
        if self.is_full:
            self.msg("Threads list if full, clear some entries before trying to add new ones.")
            return
        for key, value in self.list[self.state].items():
            ...
            if value == "":
                ...
                self.msg(f"New thread added: '{text}'")
    ...
    def clear_all_items(self):
        for key, value in self.list.items():
            for k, v in value.items():
                self.list[key][k] = ""
        self.is_full = False
        self.state = list_states[0]
        self.count = 0
        self.msg("All threads removed.")
    ...
    def retrieve_thread_from_roll(self, roll):
        self.msg(f"retrieving thread from roll: {roll}")
        state = list_states[0]
        ...
        if self.list[state][key] == "":
            ...
            if self.list[state][key] == "":
                self.msg("Threads list is empty.")
                return
    ...
    def roll(self):
        ...
        if self.state == "initial":
            ...
            thread = self.retrieve_thread_from_roll(result)
        elif self.state == "D4":
            d4 = D4()
            d10 = D10()
